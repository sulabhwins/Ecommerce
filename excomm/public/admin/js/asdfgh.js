...
$(document).on("click", ".updateSubAdminStatus", function () {
    var $subadmin = $(this);
    var status = $subadmin.find("i").data("status");
    var subadmin_id = $subadmin.data("subadmin-id");
    ...
    $.ajax({
        ...
        success: function (resp) {
            if (resp.status == 0) {
                $subadmin.html("<i class='fas fa-toggle-off' style='color:grey' data-status='Inactive'></i>");
            } else if (resp.status == 1) {
                $subadmin.html("<i class='fas fa-toggle-on' style='color:#3f6ed3' data-status='Active'></i>");
            }
        },
        error: function () {
            alert("Error");
    ...
    $(document).on("click", ".updateCategoriesStatus", function () {
        ...
        $.ajax({
            ...
            success: function (resp) {
                if (resp.status == 0) {
                    $category_id.html("<i class='fas fa-toggle-off' style='color:grey' data-status='Inactive'></i>");
                } else if (resp.status == 1) {
                    $category_id.html("<i class='fas fa-toggle-on' style='color:#3f6ed3' data-status='Active'></i>");
                }
            },
